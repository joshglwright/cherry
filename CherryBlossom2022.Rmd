...
Notice that the data abruptly cuts off at 0 throughout the years. This is likely due to missing values. We substitute the monthly average temperature when there are 0s. Kyoto is a very warm city and rarely dips below 0 degrees Celsius, so we shouldn't lose too much information with this substitution. We're missing a month of data for September 2009, but that shouldn't be too much of a problem for predicting 2022 and onward.

```{r}
# Substitute 0s for monthly averages
jp_temp2 <- jp_temp %>%
  mutate(year=year(date), month=month(date)) %>%
  group_by(year,month) %>%
  mutate(monthavg=mean(tavg), tavg=ifelse(tavg==0,monthavg,tavg))
...
Based on the above result, we estimate the peak bloom date by finding the earliest date for which the predicted growth stage is greater than or equal to 8.02.
