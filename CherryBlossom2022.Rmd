...
Similar to DC, we obtained the historical weather data for Vancouver via DayMet. However, this dataset only goes to December 2019, so we use `rnoaa` to obtain the remaining data up to Jan 2022.
...
# Liestal
The cherry tree in Liestal is a different species from the ones in DC and Vancouver. Ideally, we would want to fit a new growth model using historical phenological stages. Unfortunately, such a dataset is not readily available. So we use the same growth model from DC to calculate the mean growth stage for historical peak bloom dates. We use the average growth stage as the cutoff for our prediction.
...
testdata <- cherry %>% 
  ...
  filter(year >= 1980 & location=='liestal') %>%
  select(GDD)

mean(predict(growth_model,newdata=testdata),na.rm=TRUE)
```

We can approximate the peak bloom for the cherry trees in Liestal (as defined by MeteoSwiss) using the growth model for the trees in DC if we use 7.39 as a threshold for the predicted growth stage. In order words, we estimate the peak bloom date by finding the earliest date for which the predicted growth stage is greater than or equal to 7.39.
...
# Kyoto
Similar to predicting peak bloom dates for Liestal, the peak bloom dates for Kyoto (as defined by a local newspaper in Arashiyama) need to be approximated using the growth model for the DC cherry trees. 
...
plot(jp_temp$date,jp_temp$tavg,type="l")
```

Notice that the data abruptly cuts off at 0 throughout the years. This is likely due to missing values. We remove 0s to NAs so that the algorithm knows that the value is actually missing. Kyoto is a very warm city and rarely dips below 0 degrees Celsius, so we shouldn't lose too much information with this substitution. We're missing a month of data for September 2009, but that shouldn't be too much of a problem for predicting 2022 and onward.
...
plot(jp_temp2$date,jp_temp2$tavg,type="l")
```

With the substitution, the odd behavior is gone. We can proceed to calculate GDD for Kyoto.
...
Based on the above result, we estimate the peak bloom date by finding the earliest date for which the predicted growth stage is greater than or equal to 8.26.
...
<!-- There's something weird going on with the last few months of data for Kyoto. As shown in the plot above. When comparing the time period between August and January for 2020 versus 2021, we see that the plot for 2021 proceeds horizontally and abruptly drops compared to the 2020 plot, which shows are more gradual downward trend.  -->

<!-- ```{r echo=FALSE,message=FALSE} -->
<!-- jp_temp2 %>% filter(date >= "2021-08-01" & date <= "2021-12-31") %>%  -->
<!--   ungroup() %>% select(date,tavg) %>% ggplot(aes(x=date)) + geom_line(aes(y=tavg)) + -->
<!--   theme_minimal() + -->
<!--   ggtitle("Temperature Plot for August through December (2021)") + -->
<!--   xlab("Date") + -->
<!--   ylab("Degree Celsius") + -->
<!--   theme(plot.title=element_text(hjust=0.5)) -->

<!-- jp_temp2 %>% filter(date >= "2020-08-01" & date <= "2020-12-31") %>%  -->
<!--   ungroup() %>% select(date,tavg) %>% ggplot(aes(x=date)) + geom_line(aes(y=tavg)) + -->
<!--   theme_minimal() + -->
<!--   ggtitle("Temperature Plot for August through December (2020)") + -->
<!--   xlab("Date") + -->
<!--   ylab("Degree Celsius") + -->
<!--   theme(plot.title=element_text(hjust=0.5)) -->

<!-- ``` -->
<!-- While not impossible, the historical weather available through [Weather Underground](https://www.wunderground.com/history/monthly/jp/itami-shi/RJOO) shows that the temperature drop was not so drastic. As this sudden drop will significantly affect predictions, we deemed the 2021 data for Kyoto unreliable and removed the records. -->

<!-- ```{r} -->
<!-- # Create time series -->
<!-- ts_temp2 <- ts(GDD_final$tavg[50:(nrow(GDD_final)-365)],frequency = 365, -->
<!--              #Only start counting from day 50 -->
<!--              start=c(1980,50),end=c(2020,365)) -->
...
# Final predictions
The summary table below shows the final predictions for the four locations from 2022 to 2031.
```{r, echo=FALSE}
results <- tibble(year=seq(2022,2031),kyoto=forecasts_jp$doy,liestal=forecasts_li$doy,
                  washingtondc=forecasts_dc$doy,vancouver=forecasts_ca$doy)
...
# Closing thoughts
If historical phenological stages were recorded for all locations in this competition, we would have been able to fit a growth model for each species and location. This would have allowed us to create more accurate models. Additionally, weather data was extremely limited or unreliable for locations outside the United States. If historical weather data was free and readily available to the public, we would have been able to create a better time series model for GDD. Lastly, using weather forecasts from meteorologists will likely improve the accuracy of the peak bloom predictions.
