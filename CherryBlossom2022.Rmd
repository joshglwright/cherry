...
temp_list <- lapply(locs,getTemperature,date_min = "1980-01-01", date_max = "2022-01-31")

# Data for Liestal (missing 2015 and half of 2016. Take them out)
# Data for Kyoto (missing Sep 2001. Take out 2001.)
...
alldates <- tibble(date=seq.Date(from=as.Date('1980-01-01'),to=as.Date('2022-01-31'),by='day'))
...
We can approximate the peak bloom for the cherry trees in Liestal (as defined by MeteoSwiss) using the growth model for the trees in DC if we use 7.4 as a threshold for the predicted growth stage. In order words, we estimate the peak bloom date by finding the earliest date for which the predicted growth stage rounds to 7.4.

```{r, cache=TRUE, echo=FALSE}
# Create time series
ts_gdd <- ts(GDD_final$tavg[50:nrow(GDD_final)],frequency = 365,
             #Only start counting GDD from day 50 & 2022 li data only goes to January
             start=c(1980,50),end=c(2022,31)) 
ts_gdd %>%
...
if (file.exists('pred_li.rds')){
  ...
  ts_fit <- auto.arima(ts_gdd, ic = 'aicc',D=1,start.P=0,start.Q=0,max.Q=0,max.P=0)
  ar <- ts_fit$arma[1:2]
  ma <- ts_fit$arma[3:4]
  dd <- ts_fit$arma[5:7]
  ...
  # Use result from fitted time series to predict GDD until May 2031
  preddays <- difftime("2031-05-31","2022-02-01",units="days")
  pred_li <- sarima.for(ts_gdd,as.numeric(preddays),ar[1],dd[2],ar[2],ma[1],dd[3],ma[2],dd[1])
  saveRDS(pred_li,here("pred_li.rds"))
}

# Forecasted GDD and growth stage
newdata <- tibble(date=seq.Date(as.Date("2022-02-01"),length.out=length(pred_li$pred),by="day"),
                  doy=yday(date),year=year(date)) %>%
  mutate(NewYear=year(date %m-% period("49 day"))) %>%
  mutate(DD = ifelse(tavg - thresh > 0, tavg - thresh, 0)) %>%
  group_by(NewYear) %>%
...
forecasts_li <- newdata %>% mutate(pred.gs=future_growthstage) %>%
  group_by(year) %>%
  filter(doy >= 50 & doy <= 200) %>%
  filter(round(pred.gs,digits=1) >= 7.4) %>%
  filter(row_number()==1) %>%
  select(year,doy)
...
Based on the above result, we estimate the peak bloom date by finding the earliest date for which the predicted growth stage rounds to 8.

```{r, cache=TRUE, echo=FALSE}
# Create time series
ts_gdd <- ts(GDD_final$tavg[50:nrow(GDD_final)],frequency = 365,
             #Only start counting GDD from day 50 & 2022 li data only goes to January
             start=c(2017,50),end=c(2022,31)) 
...
if (file.exists('pred_jp.rds')){
  ...
  # Use result from fitted time series to predict GDD until May 2031
  preddays <- difftime("2031-05-31","2022-02-01",units="days")
  pred_jp <- sarima.for(ts_gdd,as.numeric(preddays),ar[1],dd[2],ar[2],ma[1],dd[3],ma[2],dd[1])
  saveRDS(pred_jp,here("pred_jp.rds"))
}

# Forecasted GDD and growth stage
newdata <- tibble(date=seq.Date(as.Date("2022-02-01"),length.out=length(pred_jp$pred),by="day"),
                  doy=yday(date),year=year(date)) %>%
  mutate(NewYear=year(date %m-% period("49 day"))) %>%
  mutate(DD = ifelse(tavg - thresh > 0, tavg - thresh, 0)) %>%
  group_by(NewYear) %>%
...
forecasts_jp <- newdata %>% mutate(pred.gs=future_growthstage) %>%
  group_by(year) %>%
  filter(doy >= 50 & doy <= 200) %>%
  filter(round(pred.gs,digits=1) >= 8) %>%
  filter(row_number()==1) %>%
  select(year,doy)
