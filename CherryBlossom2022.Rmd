...
# Predicting peak bloom dates for DC
...
# Predicting peak bloom dates for Liestal
...
temp_list <- lapply(locs,getTemperature,date_min = "2017-01-01", date_max = "2022-01-31")
...
li_temp <- temps %>% filter(city == 'Liestal') %>%
...
testdata <- cherry %>% 
  ...
  filter(year >= 2017 & location=='liestal') %>%
  select(GDD)

mean(predict(growth_model,newdata=testdata))
```

We can approximate the peak bloom for the cherry trees in Liestal (as defined by MeteoSwiss) using the growth model for the trees in DC if we use 6.8 as a threshold for the predicted growth stage. In order words, we estimate the peak bloom date by finding the earliest date for which the predicted growth stage rounds to 6.8.
...
newdata <- tibble(date=seq.Date(as.Date("2022-02-01"),length.out=length(pred_li$pred),by="day"),
                    GDD=pred_li$pred,doy=yday(date),year=year(date))
future_growthstage <- predict(growth_model,newdata=newdata)

forecasts_li <- newdata %>% mutate(pred.gs=future_growthstage) %>%
  group_by(year) %>%
  filter(doy >= 50 & doy <= 200) %>%
  filter(round(pred.gs,digits=1) >= 6.8) %>%
  filter(row_number()==1) %>%
  select(year,doy)
...
# Predicting peak bloom dates for Kyoto
...
testdata <- cherry %>% 
  left_join(GDD_final %>% select(date,GDD), by=c('bloom_date'='date')) %>%
  filter(year >= 2017 & location=='kyoto') %>%
  select(GDD)

mean(predict(growth_model,newdata=testdata))
```

Based on the above result, we estimate the peak bloom date by finding the earliest date for which the predicted growth stage rounds to 8.2.
...
if (file.exists('pred_jp.rds')){
  pred_li <- readRDS(here("pred_jp.rds"))
} else{
  # Fit time series to GDD
  ts_fit <- auto.arima(ts_gdd, ic = 'aicc')
  ar <- ts_fit$arma[1:2]
...
newdata <- tibble(date=seq.Date(as.Date("2022-02-01"),length.out=length(pred_jp$pred),by="day"),
                    GDD=pred_jp$pred,doy=yday(date),year=year(date))
future_growthstage <- predict(growth_model,newdata=newdata)

forecasts_jp <- newdata %>% mutate(pred.gs=future_growthstage) %>%
  group_by(year) %>%
  filter(doy >= 50 & doy <= 200) %>%
  filter(round(pred.gs,digits=1) >= 8.2) %>%
  filter(row_number()==1) %>%
  select(year,doy)
