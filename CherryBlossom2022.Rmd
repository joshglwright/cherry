...
We can approximate the peak bloom for the cherry trees in Liestal (as defined by MeteoSwiss) using the growth model for the trees in DC if we use `r li_thresh` as a threshold for the predicted growth stage. In order words, we estimate the peak bloom date by finding the earliest date for which the predicted growth stage is greater than or equal to `r li_thresh`.
...
Based on the above result, we estimate the peak bloom date by finding the earliest date for which the predicted growth stage is greater than or equal to `r jp_thresh`.
